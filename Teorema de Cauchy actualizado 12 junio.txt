...
--Esta es la proposicion clave que usa el teorema para demostrar que el cardinal de las orbitas de C sobre S divide a p, la demostramos usando el teorema ya disponible en matlib
theorem proposition2_1 (G S : Type) [Group G] [Fintype G] [MulAction G S] (s:S):  
     (MulAction.stabilizer G s).index = Nat.card (MulAction.orbit G s) := by
     rw [Subgroup.index_eq_card]
     exact Nat.card_congr (MulAction.orbitEquivQuotientStabilizer G s).symm
...
theorem division_orbita (G S : Type) (n : ℕ) [Group G] [Fintype G] [Fintype S] [MulAction G S] (h_prim : Nat.Prime n) (h_zero : n≠ 0) (h_card : Nat.card G = n) (h_todo_igual : ∀ (s:S), (Nat.card (MulAction.orbit G s)) = n) : ∀ (s:S), (Nat.card (MulAction.orbit G s)) ∣ Nat.card S := by
  classical --Si no pongo esto, no me compila el ω.out
  intro s
  rw[h_todo_igual]
  rw [← Fintype.card_eq_nat_card]
  have h_equiv := MulAction.selfEquivSigmaOrbits G S --Esto es lo que nos dice que S es una union disjunta de las orbitas de G sobre S, como se explica en el marco teórico
  have h_card_S := Fintype.card_congr h_equiv
  rw [h_card_S, Fintype.card_sigma]
  apply Finset.dvd_sum --Con esto cambiamos el objetivo a mostrar que n divide al cada elemento de la suma, es decir el cardinal de cada orbita
  intro ω _
  have h_orb_size : Fintype.card (MulAction.orbit G ω.out) = n := by
    have hx := h_todo_igual ω.out
    rw [← Fintype.card_eq_nat_card] at hx
    exact hx
  rw [h_orb_size]


--Defino aqui un resultado que me va a ayudar a simpplificar la parte del cardinal del conjunto |S|
--Lo que queremos mostrar a Lean con este teorema es que hay una biyeccion entre una coleccion de (n-1) elementos de G y T, lo cual viene a decir que cualquier elemento de T depende de (n-1) elementos de G, y es evidente de ver en papel pero en Lean es más complicado

noncomputable def equiv_T (G : Type) [Group G] [Fintype G] (n : ℕ) (hn : n > 0) : (Fin (n - 1) → G) ≃ { t : (Fin n → G) // (List.ofFn t).prod = 1 } := by
  classical
  exact{
    toFun := fun f => --Tenemos un f de (Fin (n - 1) → G) y vamos a mostrar que a partir de el podemos construir un elemento de T
      let t : Fin n → G := fun i => 
        if h : i.val < n - 1 then --Si el i del t que queremos es < n-1 entonce, nos vale el valor correspondiente en f
          f ⟨i.val, h⟩
        else
          (List.ofFn f).prod⁻¹ --Sino, lo construimos usando el producto inverso de f
      ⟨t, by 
        have h_inverso : ((List.ofFn f).prod)*(List.ofFn f).prod⁻¹ = 1 := by exact mul_inv_cancel (List.ofFn f).prod
        have t_igual : (List.ofFn t).prod = ((List.ofFn f).prod)*(List.ofFn f).prod⁻¹ := by
          have h_list : List.ofFn t = List.ofFn f ++ [(List.ofFn f).prod⁻¹] := by
            have h_len : (List.ofFn t).length = (List.ofFn f ++ [(List.ofFn f).prod⁻¹]).length := by
              rw [List.length_ofFn, List.length_append, List.length_ofFn, List.length_singleton]
              omega
            apply List.ext_get h_len
            intro i hi1 hi2
            rw [List.length_ofFn] at hi1
            by_cases h_lt : i < n - 1
            · rw [List.get_ofFn]
              dsimp [t]
              rw [dif_pos h_lt]
          ...
          rw [h_list]
          rw[List.prod_append]
          exact Eq.symm (eq_mul_of_inv_mul_eq (congrArg (HMul.hMul (List.ofFn f).prod⁻¹) h_inverso))
        rw[← t_igual] at h_inverso
        exact h_inverso  ⟩  
    invFun := fun ⟨t, _⟩ => --Tenemos un t de T
      fun i => t ⟨i.val, Nat.lt_of_lt_of_le i.isLt (Nat.sub_le n 1)⟩ --Simplemente tomamos los n-1 primeros elementos
    --Aqui mostramos que invFun(toFun(x)) = x
    left_inv := by
      intro f
      ext i
      dsimp
      ...
      exact if_pos h_lt
    --Aqui mostramos que toFun(invFun(y))=y
    right_inv := by
      intro ⟨t,h⟩
      ext i
      dsimp
      split_ifs with h_lt
      rfl
      let f : Fin (n - 1) → G := fun j => t ⟨j.val, Nat.lt_of_lt_of_le j.isLt (Nat.sub_le n 1)⟩
      have hi : i.val = n - 1 := by omega
      have h_list : List.ofFn t = List.ofFn f ++ [t i] := by
        have h_len : (List.ofFn t).length = (List.ofFn f ++ [t i]).length := by
          rw [List.length_ofFn, List.length_append, List.length_ofFn, List.length_singleton]
          omega
        apply List.ext_get h_len
        intro j hj1 hj2
        rw [List.length_ofFn] at hj1
        by_cases hj_lt : j < n - 1
        · rw [List.get_ofFn]
          simp [hj_lt]
          exact (congrFun rfl ∘ t) i
      ...
      have h_prod := h
      rw [h_list] at h_prod
      rw [List.prod_append, List.prod_cons, List.prod_nil, mul_one] at h_prod
      have h_inv := congrArg (fun x => (List.ofFn f).prod⁻¹ * x) h_prod --Multiplicamos ambos lados por (List.ofFn f).prod⁻¹
      rw [inv_mul_cancel_left] at h_inv
      rw [mul_one] at h_inv
      exact h_inv.symm
...
--Definimos aqui otro resultado para simplificar la demostracion de que |S|=|T|-1, simplemente definimos una equivalencia que dice que el cardinal de T equivale a S más otro elemento, pongo noncomputable porque sino con el classical me da muchos problemas

noncomputable def equiv_S (G : Type) [Group G] [Fintype G] (n : ℕ) : { t : (Fin n → G) // (List.ofFn t).prod = 1 } ≃ Option { s : (Fin n → G) // (List.ofFn s).prod = 1 ∧ s ≠ (fun _ => 1)} := by
  classical
  exact{
    --Miramos que un elemento de { t // (List.ofFn t).prod = 1 } este en Option { s // (List.ofFn s).prod = 1 ∧ s ≠ fun x => 1 }
    toFun := fun ⟨t, ht⟩ => 
      if h : t = (fun _ => 1) then 
        none 
      else 
        some ⟨t, ⟨ht, h⟩⟩
    --Miramos que un elemento de Option { s // (List.ofFn s).prod = 1 ∧ s ≠ fun x => 1 } este en { t // (List.ofFn t).prod = 1 }
    invFun := fun
      | none => ⟨fun _ => 1, by 
          ...
          rfl⟩ --Si el elemento es el none, entonces creamos el vector de unos
      | some ⟨s, ⟨hs, h_1⟩⟩ => ⟨s, hs⟩ --Si no lo es, entonces esta en { t // (List.ofFn t).prod = 1 }
       
    left_inv := by
    --Aqui mostramos que invFun(toFun(x)) = x
      intro ⟨t, ht⟩
      dsimp
      split_ifs with h --Dividimos los dos casos del if de toFun
      · subst h --En el caso en el que es (1,1,1..) lo enviamos a none, y la funcion invFun(None) = (1,1,1...)
        ext
        rfl
      · rfl --En el caso contrario lo enviamos a un vector que no sea el unitario y invFun(vector que no sea el unitario) = vector que no sea el unitario
    right_inv := by
    --Aqui mostramos que toFun(invFun(y)) = x
      intro x --Tomamos un x de Option { s // (List.ofFn s).prod = 1 ∧ s ≠ fun x => 1 } y estudiamos los dos casos si es none y sino
      cases x with
      | none => --Si es none invFun(None) = (1,1,1,....) y Fun(1,1,1,....) = None
        dsimp
        have h_id : (fun (i : Fin n) => (1 : G)) = (fun _ => 1) := rfl
        rw [dif_pos h_id]
      | some s_val => 
        obtain ⟨s, hs, h_no_uno⟩ := s_val
        dsimp
        rw [dif_neg h_no_uno]
  }
...
def no_div_potencia_1 (a b : ℕ  ) (h_a : a > 1) (h_b : b > 0) (h_div_ab :  a ∣ b) : ¬ (a ∣ b-1) := by
   by_contra! h_absurdo
   have h_falso_1 : a ∣ (b - (b-1)) := by exact Nat.dvd_sub h_div_ab h_absurdo
   have h_falso_2 : (b - (b-1)) =1 := by exact Nat.sub_sub_self h_b
   rw[h_falso_2] at h_falso_1
   have h_falso : (a > 1) ∧ (a ∣ 1) := by exact ⟨h_a, h_falso_1⟩
   have h_contradict : (a ∣ 1) → (a=1) := by exact fun a_1 => Nat.eq_one_of_dvd_one h_falso_1
   have h_final : a = 1 := by exact Nat.eq_one_of_dvd_one h_falso_1
   have h_final_2 : a > 1 := by exact Nat.lt_of_succ_le h_a
   rw[h_final] at h_final_2
   exact (lt_self_iff_false 1).mp h_final_2
...
theorem cauchy_theorem (p : ℤ) (G : Type) [Group G] [Fintype G] (hp : Prime p) (h_no_one : p ≠ 0) (h_div : p.natAbs ∣ Fintype.card G) : 
  ...
  haveI h_card_T : Fintype.card T = (Fintype.card G)^(n - 1) := by
    have h_n_cero : n > 0 := by exact Int.natAbs_pos.mpr h_no_one
    --Tenemos que mostrar que un e en T tiene (n-1) elementos libres, aqui podemos usar el resultado equiv_T
    calc Fintype.card T
      _ = Fintype.card (Fin (n - 1) → G) := Fintype.card_congr (equiv_T G n h_n_cero).symm
  ...
  have hfinal: ∃ g:G, orderOf g = p.natAbs := by
    have s_g : ∃ g:G, ∀ i, s_final.val i = g := by
      (expose_names; exact Exists.intro (s_final.val 0) h_todos_iguales)
    rcases s_final with ⟨valor, propiedad_1,propiedad_2⟩
    rcases s_g with⟨g, prop⟩
    have h_prod_g : (List.ofFn (fun (_ : Fin n) => g)).prod = 1 := by
      have h_ig_valor:  (List.ofFn (fun (_ : Fin n) => g)).prod = (List.ofFn valor).prod := by
        have h1 : (fun (_ : Fin n) => g) = valor := by
          ext i
          exact (prop i).symm
        rw[h1]
      rw[h_ig_valor]
      exact propiedad_1
    have h_final_g : g^n = 1 := by
      simpa using h_prod_g
    
    have h_g_no_uno : g ≠ 1 := by
      by_contra! h_absurdo
      have h: (fun (_ : Fin n) => g) = (fun (_ : Fin n) => 1) := by rw[h_absurdo]
      have h_1_falso: valor = (fun (_ : Fin n) => g) := by exact (Set.eqOn_univ valor fun x => g).mp fun ⦃x⦄ a => prop x
      have h_2_falso: valor = (fun (_ : Fin n) => 1) := by
        rw[h] at h_1_falso
        exact h_1_falso
      have h_final_falso : valor = vector_unitario := by exact List.ofFn_inj.mp (congrArg List.ofFn h_2_falso)
      exact propiedad_2 h_final_falso
    have primo : Fact (Nat.Prime n) := by (expose_names; exact { out := this_1 })
    have h_ultima: orderOf g = n := by
      exact orderOf_eq_prime h_final_g h_g_no_uno
    use g
