...
theorem cauchy_theorem (p : ℤ) (G : Type) [Group G] [Fintype G] (hp : Prime p) (h_no_one : p ≠ 0) (h_div : p.natAbs ∣ Fintype.card G) : 
  ...
  letI existe_s : ∃ s:S , ∀ c:C, (fun i => s.val (i + c_fin c)) = s.val := by exact Exists.imp (fun a a_1 => a_1) (s_clave h_existe_card_1)
  ...
  have hfinal: ∃ g:G, orderOf g = p.natAbs := by
    ...
    have s_g : ∃ g:G, ∀ i, s_final.val i = g := by
      use s_final.val 0
      intro i
      let c : C := Multiplicative.ofAdd (i.val : ZMod n)
      have h_eval := congr_fun (h_invariante c) 0
      have h_idx : 0 + c_fin c = i := by
        apply Fin.ext
        have h : 0 + c_fin c = c_fin c := by exact Fin.zero_add (c_fin c)
        rw[h]
        change (Multiplicative.toAdd c).val = i
        sorry
    ...
    rcases s_g with ⟨g_final, h_g⟩
    have h_orden : orderOf g_final = n := by
      have h_mul : (List.ofFn s_final.val).prod = 1 := by
        rcases s_final with ⟨valor, propiedad_1,propiedad_2⟩
        exact propiedad_1
      let s_0 := s_final.val 0
      have h_igual : s_0 = g_final := by exact ((fun a => h_g 0) ∘ estabilizador) s_final
      have h_orden_g : g_final^n = 1 := by
        rcases s_final with ⟨valor, propiedad_1,propiedad_2⟩
        rw[← h_igual]
        have h1 : ∀ i, valor i = s_0 := by exact fun i => cast (congrArg (Eq (valor i)) (id (Eq.symm h_igual))) (h_g i)
        sorry
      sorry
    sorry      
